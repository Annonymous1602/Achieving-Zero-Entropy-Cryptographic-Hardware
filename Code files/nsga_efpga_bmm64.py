...
TEMPLATE_FILE = 'customFPGATemplate_bmm64_128_256.xml'

# Only BOOTH is a hard block now.
BLOCKS = ['BOOTH']
BMM64_EXPECTED_BOOTH = 9
BLOCK_HEIGHT = 4
BLOCKS_COUNT = {}
MAX_BLOCKS_COUNT = {}
...
def read_resource_block_limits(resource_dir, benchmark):
    ...
    if not os.path.exists(path):
        print(f'[WARN] Resource file not found: {path}. Using BOOTH 9 for bmm_64.')
        return {'BOOTH': 9}
    ...
    if counts['BOOTH'] <= 0:
        print('[WARN] BOOTH count is 0. For current bmm_64 booth-hardblock flow, expected BOOTH 9.')
    return counts

def build_allowed_locations(layout):
    """Return BOOTH placement sites that are safe for VPR auto_layout.

    With auto_layout the final grid width/height changes with aspect_ratio.
    Large fixed_70x70 coordinates such as x=56 can become illegal.  For this
    bmm_64 BOOTH-only flow we only need 9 BOOTH sites, so we intentionally keep
    the evolved coordinate choices inside a compact interior region:

        x in {1, 3, 5, 7}
        y in {1, 10, 19}

    These 12 choices are enough for 9 non-overlapping BOOTH blocks, fit inside
    the narrow auto-layout grids observed earlier, and still allow NSGA to vary
    the BOOTH x/y placement.
    """
    xs = [1, 3, 5, 7]
    ys = [1, 10, 19]
...
def build_booth_list(ind_data, booth_count):
    """Use the evolved BOOTH coordinates from the NSGA chromosome.

    ind_data layout is:
      [aspect_gene, booth0_xy, booth1_xy, ..., boothN_xy, clb_type_gene]
    This restores the old behavior where each individual has its own hard-block
    x/y placement. The repair operator keeps the sites non-overlapping, and
    build_allowed_locations() keeps them inside the auto_layout-safe region.
    """
    booths = []
    for i in range(1, 1 + booth_count):
        x, y = ind_data[i]
        booths.append([int(x), int(y), PRIORITY_H_BLOCKS])
    return booths

def profile_required_hard_blocks():
    """Use the resource file as an upper bound, then profile exact BOOTH usage.

    If project/resourceBlocks/reconfig_bmm_64.txt says BOOTH 10, this profiling
    pass renders up to 10 BOOTH sites, runs VTR once, and parses how many BOOTH
    primitives the bmm_64 netlist actually uses. For the current bmm_64 design
    this becomes BOOTH 9, so later NSGA runs use 9 genes/sites, not 10.
    """
    ...
    if upper_count > len(allowed):
        print(f'[WARN] Resource file asks for BOOTH {upper_count}, but the auto-layout-safe placement pool has only {len(allowed)} sites. Profiling with {len(allowed)} sites.')
    profile_site_count = min(upper_count, len(allowed))

    booths = [[x, y, PRIORITY_H_BLOCKS]
              for x, y in pick_non_overlapping_sites(profile_site_count, allowed)]
    ...
    render_architecture_xml(xml_path, 1.0, 'CLB_LUT6', booths)
    ...
    if status != 0:
        fallback = min(upper_count, BMM64_EXPECTED_BOOTH) if upper_count > 0 else 0
        print('[WARN] Resource profiling run failed. Falling back to safe benchmark requirement.')
        print(f'Upper-bound hard blocks from resource file: {upper}')
        print(f"Profiled required hard blocks for current design: {{'BOOTH': {fallback}}}")
        return upper, {'BOOTH': fallback}

    delay, power, metrics = parse_task_outputs(task_dir)
    parsed = metric_count(metrics.get('booth_blocks'), 0)

    if parsed > 0:
        required_count = min(upper_count, parsed)
    else:
        required_count = min(upper_count, BMM64_EXPECTED_BOOTH) if upper_count > 0 else 0
        print(f'[WARN] Could not parse BOOTH usage from profiling VPR output. Using fallback BOOTH {required_count}.')
    ...
    if parsed > upper_count > 0:
        print(f'[WARN] Design appears to require BOOTH {parsed}, but resource file upper bound is BOOTH {upper_count}. Using upper bound {upper_count}.')
...
class FPGA_Architecture(Problem):
    ...
    def evaluateArchitecture(self, x_raw, Z):
        ...
        if overlap > 0:
            print(f'  [DISCARD] Residual overlap ({overlap}) for individual {Z}')
            clb_info = dict(ind_id=Z, aspect_ratio=aspect_ratio, clb_type=clb_type_idx,
                            clb_name=clb_name, delay=100.0, power=10.0, status='overlap_violation')
            return [100.0 / self.delay, 10.0 / self.power], [float(overlap)], clb_info
